...
<body onload="">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">风格列表&nbsp<button type="button" class="btn btn-success" data-toggle="modal"
					data-target="#insstyleModel"  onclick="javascript:insstyle()">添加风格</button></div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					...
					<div class="col-sm-3" style="padding-left: 0; margin-bottom: 15px">
                      <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1')">
					...
					<div class="col-sm-3">
					     <div class="col-sm-10">
					     <input class="form-control" placeholder="请输入风格名" id="search_input" value="${name }">
					    </div>
					    <div class="col-sm-2">
					    <button type="button" class="btn btn-success"  onclick="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1')">搜索</button>
					    </div>
					</div>
						...
						<table
								...
								<%-- <th>orderindex<span>&nbsp</span>
									 <c:choose>
									  <c:when test="${order== '0' && orderConditions=='orderindex'}">
									     <a class="down" href="javascript:changeTriangle('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1','1','orderindex')" style="text-decoration:none;">▼</a>   
									  </c:when>
									  <c:when test="${order== '1'  && orderConditions=='orderindex'}">
									     <a class="up" href="javascript:changeTriangle('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1','0','orderindex')"   style="text-decoration:none;">▲</a>
									  </c:when>
									  <c:otherwise>
									     <a class="up_down" href="javascript:changeTriangle('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1','1','orderindex')"   style="text-decoration:none;">◊</a>									     
									  </c:otherwise>
									</c:choose> 
									</th>
									<th>bid<span>&nbsp</span>
									 <c:choose>
									  <c:when test="${order== '0' && orderConditions=='bid'}">
									     <a class="down" href="javascript:changeTriangle('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1','1','bid')" style="text-decoration:none;">▼</a>   
									  </c:when>
									  <c:when test="${order== '1'  && orderConditions=='bid'}">
									     <a class="up" href="javascript:changeTriangle('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1','0','bid')"   style="text-decoration:none;">▲</a>
									  </c:when>
									  <c:otherwise>
									     <a class="up_down" href="javascript:changeTriangle('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1','1','bid')"   style="text-decoration:none;">◊</a>									     
									  </c:otherwise>
									</c:choose> 
									</th> --%>
									<th>styleid</th>
							...
							<tbody>
								<c:forEach items="${mallStyleList}" var="list"
									varStatus="ids">
									<tr id="${list.styleid}">
									   <td >${list.styleid}</td>
									   <td >${list.name}</td>
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5"/>&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/mallmanage/mallstylelist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
	...
	<div class="modal fade" id="insstyleModel" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-body" >
					...
					<div class="form-group">
						...
						<input class="form-control" placeholder="输入风格名" id="ins_name" onblur="javacript:styleNameCheck()"
							value="" >
	...
	<script src="../js/mallmanage/mallstylelist.js"></script> 
