...
function pageGoto(url, adminGuid,page){ 
	...
	$("#page-wrapper").load(url + "?userGuid=" + adminGuid+ "&page="+page+"&limit="+limit+"&order="+order+"&categoryid="+categoryid);	
}
function inputPageGoto(url, adminGuid,totalPage){
	...
	if(reg.test(inputPage)==true && (parseInt(inputPage)<=parseInt(totalPage))){
		...
		var order= $("#triangleHidden").val();
		var categoryid=$("#categorySelect").val();
		$("#page-wrapper").load(url + "?userGuid=" + adminGuid+ "&page="+inputPage+"&limit="+limit+"&order="+order+"&categoryid="+categoryid);
...
<body onload="">
	...
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">精选动态分类管理</h1>
		</div>
		<!-- /.col-lg-12 -->
	</div>
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">精选动态分类列表</div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					...
					<div class="col-sm-3" style="padding-left: 0; margin-bottom: 15px">
                      <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','1')">
                      ...
                      <div>
							  ...
							  <select class="form-control" id="categorySelect"  onchange="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','1')">
                                   <c:choose>
                                       <c:when test="${categoryid==1 }">
                                          <option value="1" selected>用车经验</option>
                                       </c:when>
                                       <c:otherwise>
                                        <option value="1">用车经验</option>
                                       </c:otherwise>
                                   </c:choose>
							      <c:choose>
                                       <c:when test="${categoryid==2 }">
                                          <option value="2" selected>摩旅游记</option>
                                       </c:when>
                                       <c:otherwise>
                                        <option value="2">摩旅游记</option>
                                       </c:otherwise>
                                   </c:choose>
                                   <c:choose>
                                       <c:when test="${categoryid==3 }">
                                         <option value="3" selected>评测</option>
                                       </c:when>
                                       <c:otherwise>
                                       <option value="3">评测</option>
                                       </c:otherwise>
                                   </c:choose>
                                   <c:choose>
                                       <c:when test="${categoryid==4 }">
                                          <option value="4" selected>生活文化</option>
                                       </c:when>
                                       <c:otherwise>
                                        <option value="4">生活文化</option>
                                       </c:otherwise>
                                   </c:choose>
                                   <c:choose>
                                       <c:when test="${categoryid==5 }">
                                          <option value="5" selected>改装</option>
                                       </c:when>
                                       <c:otherwise>
                                        <option value="5">改装</option>
                                       </c:otherwise>
                                   </c:choose>
                                   <c:choose>
                                       <c:when test="${categoryid==6 }">
                                          <option value="6" selected>竞技极限</option>
                                       </c:when>
                                       <c:otherwise>
                                        <option value="6">竞技极限</option>
                                       </c:otherwise>
                                   </c:choose>
						...
						<table
							...
							<thead>
								<tr>
									...
									<c:choose>
									  <c:when test="${order== 0}">
									     <a  class="down" href="javascript:changeTriangle('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','1','1')" style="text-decoration:none;">▼</a>
									     <input id="triangleHidden" type="hidden" value="0"/>
									  </c:when>
									  <c:otherwise>
									     <a id="triangle" class="up" href="javascript:changeTriangle('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','1','0')"   style="text-decoration:none;">▲</a>
										     ...
										     <button type="button" class="btn btn-success"
												onclick="javascript:selnews('${newsMessages.nid}')">查看动态</button>
													...
													<button type="button" class="btn btn-danger btn-lg"
														...
														onclick="javascript:delRecommenddiag('${newsMessages.nid}','${newsMessages.score }')">从池中移除</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/news/newsrecommendcategorylist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">从该精选分类移除</h4>
				</div>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" data-dismiss="modal"
						onclick="javascript:deleteFromCategory()">确认移除</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
                   ...
                   <div class="form-group">
					...
					<div class="form-group">
						<input class="form-control" placeholder="" id="update_score"
							value="">
						<input  type="hidden" value="${categoryid}" id="categoryidHidden">
					</div>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" data-dismiss="modal"
						onclick="javascript:saverecommendnewscategory()" id="savebtn">保存</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
