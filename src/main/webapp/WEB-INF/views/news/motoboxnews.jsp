...
<body onload="">
	...
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">外链动态管理</h1>
		</div>
		<!-- /.col-lg-12 -->
	</div>
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">外链动态列表</div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					<div class="dataTable_wrapper">
                      ...
                      <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','1')">
						...
						<table
							...
							<thead>
								<tr>
									...
									<c:choose>
									  <c:when test="${order== 0}">
									     <a  class="down" href="javascript:changeTriangle('/news/motoboxnews','${sessionScope.user.user_guid}','1','1')" style="text-decoration:none;">▼</a>
									     <input id="triangleHidden" type="hidden" value="0"/>
									  </c:when>
									  <c:otherwise>
									     <a id="triangle" class="up" href="javascript:changeTriangle('/news/motoboxnews','${sessionScope.user.user_guid}','1','0')"   style="text-decoration:none;">▲</a>
							...
							<tbody>
								<c:forEach items="${newsModels}" var="newsMessages"
									...
									<tr class="${newsMessages.nid}">
										...
										<td>${newsMessages.motoboxmodel.title}</td>
										<td>${newsMessages.motoboxmodel.subtitle}</td>
										<td>${newsMessages.ptimeString }</td>
										<td>${newsMessages.updatetimeString }</td>
										<td><img src='${newsMessages.motoboxmodel.titleimage}!thumb'
                                       ...
                                       <c:choose>
										...
										<td class="center">
											<button type="button" class="btn btn-success"
												onclick="javascript:selboxnews('${newsMessages.motoboxmodel.boxurl}')">查看动态</button>
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/news/motoboxnews','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
...
<div class="modal fade" id="changecategoryModel" tabindex="-1"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					   <div class="form-group">
						...
						<!--  <div class="form-group">
						    <input name="category_pool" type="checkbox" value="" />时间
						    <input name="category_pool" type="checkbox" value="" />精选
						 </div> -->
	...
	<script src="../js/news/motoboxnews.js"></script>
</body>
