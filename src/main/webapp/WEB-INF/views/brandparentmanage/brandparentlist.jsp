	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">品牌列表<span>&nbsp</span><!-- <button type="button" class="btn btn-success" data-toggle="modal" data-target="#insbrandparentModel" onclick="javascript:insbrandparent()">添加</button> --></div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					...
					<div class="col-sm-3" style="padding-left: 0; margin-bottom: 15px">
                      <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','1')">
						...
						<table
							...
							<thead>
								<tr>
									...
									<c:choose>
									  <c:when test="${order== 0}">
									     <a  class="down" href="javascript:changeTriangle('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','1','1')" style="text-decoration:none;">▼</a>
									     <input id="triangleHidden" type="hidden" value="0"/>
									  </c:when>
									  <c:otherwise>
									     <a id="triangle" class="up" href="javascript:changeTriangle('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','1','0')"   style="text-decoration:none;">▲</a>
							...
							<tbody>
								<c:forEach items="${brandparentlist}" var="bplist"
									varStatus="ids">
									<tr class="${bplist.bpid}">
									    <td>${bplist.bpid }</td>
										<td>${bplist.name}</td>
										<td><img width="100px" height="100px" src="${bplist.imgurl}"/></td>
										<c:choose>
										  <c:when test="${bplist.ishot=='1'}">
										      <td>热门</td>
										  </c:when>
										  <c:when test="${bplist.ishot=='0'}">
										      <td>不热门</td>
										  </c:when>
										</c:choose>
										<c:choose>
										  <c:when test="${bplist.state=='0'}">
										      <td>上架</td>
										  </c:when>
										  <c:when test="${bplist.state=='1'}">
										      <td>下架</td>
										  </c:when>
										</c:choose>
										<td>${bplist.orderindex}</td>
									    <td><button type="button" class="btn btn-success" data-toggle="modal" data-target="#editbrandparentModel" onclick="javascript:editbrandparent('${bplist.bpid}')">编辑</button></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/brandparentmanage/brandparentlist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
...
<!-- Modal -->
	<div class="modal fade" id="editbrandparentModel" tabindex="-1" role="dialog"
				...
				<div class="modal-body" style="height:270px">
					<div class="form-group">
					   <div class="col-sm-6">
							<div class="form-group">
								<div class="col-sm-12">
									<label>bpid</label> <input class="form-control" placeholder=""
										id="update_bpid" value="" readonly="readonly">
							...
							<div class="form-group">
								<div class="col-sm-12">
									<label>name</label> <input class="form-control" placeholder=""
										id="update_name" value="">
								</div>
							</div>
							<div class="form-group">
								<div class="col-sm-12">
									<label>orderindex</label> <input class="form-control"
										placeholder="" id="update_orderindex" value="">
								</div>
							</div>
							<div class="form-group">
								<div class="col-sm-12">
									<label>ishot</label> <select class="form-control" id="update_ishot">
	...
	<!-- Modal -->
	<div class="modal fade" id="insbrandparentModel" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<div class="form-group">
					   <div class="col-sm-6">
							...
							<div class="form-group">
								<div class="col-sm-12">
									<label>orderindex</label> <input class="form-control"
										placeholder="" id="ins_orderindex" value="">
								</div>
							</div>
							<div class="form-group">
								<div class="col-sm-12">
									<label>ishot</label> <select class="form-control" id="ins_ishot">
													<option value='0'>不热门</option>
													<option value='1'>热门</option>
												</select>
								</div>
					   ...
					   <div class="col-sm-6">
							<div class="form-group">
							   ...
							   <div>
								<input id="" type="button" value="设置图片" class="btn btn-info"
									data-toggle="modal" data-target="#titleImgModel"> <input
									type="text" value="" id="titleimgins"> <img
									id="titleimgshowins" src="" style="width: 144px; height: 144px" />
							  </div>
							</div>
						</div>
					</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal"
						id="closedelBoxModel">关闭</button>
					<button type="button" class="btn btn-primary" data-dismiss="modal"
						onclick="javascript:insConfirm()">确认添加</button>
				</div>
...
<div class="modal fade" id="titleImgModel" tabindex="-1" role="dialog"
	...
	<!-- /.modal -->
	 <script src="../js/brandparentmanage/brandparentlist.js"></script>  
</body>
