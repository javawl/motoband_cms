...
<body >
   <div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">礼物兑换管理</h1>
		</div>
	</div>
	...
	<div class="row" >
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">礼物兑换列表&nbsp&nbsp<button type="button" class="btn btn-primary btn-lg"
													onclick="javascript:addgiftexchangeNewpage('${sessionScope.user.user_guid}')">添加礼物兑换</button>&nbsp&nbsp&nbsp&nbsp&nbsp
					<button type="button" class="btn btn-info" onclick="javascript:updateVersion('giftexchangeversion')">更新礼物兑换版本</button>
													</div>
				<div class="panel-body">
					<div class="dataTable_wrapper">
                          ...
                          <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','1')">
						...
						<table
							...
							<tbody>
								<c:forEach items="${giftexchangelist}" var="exchange" varStatus="ids">
									<tr class="${exchange.exchangeid}">
									    <td>${exchange.exchangeid}</td>
										<td>${exchange.name}</td>
										<c:choose>
										    <c:when test="${fn:length(exchange.des)>30 }">
										       <td>${ fn:substring(exchange.des,0,30)} ...</td>
										    </c:when>
										    <c:otherwise>
										       <td>${exchange.des}</td>
										    </c:otherwise>
										</c:choose>
										<td>${exchange.conditiondes}</td>
										<td>${exchange.scope}</td>
										<td>${exchange.sumcount}</td>
										<td>${exchange.validitystarttimeString}</td>
										<td>${exchange.validityendtimeString}</td>
										<%-- <td>${exchange.buserids}</td> --%>
										<td>${exchange.giftName}</td>
										<td>${exchange.giftcount}</td>
										<c:choose>
										    <c:when test="${fn:length(exchange.remind)>30 }">
										       <td>${ fn:substring(exchange.remind,0,30)} ...</td>
										    </c:when>
										    <c:otherwise>
										       <td>${exchange.remind}</td>
										    </c:otherwise>
										</c:choose>
										
										<td><img src="${exchange.pic}!thumb" width="100px" height="100px"></td>
										
										<c:choose>
										  <c:when test="${exchange.state==0}">
										       <td>下架</td>
										  </c:when>
										  <c:when test="${exchange.state==1}">
										       <td>上架</td>
										  </c:when>
										  <c:otherwise>
										        <td>未知</td>
										...
										<td class="center">
										    ...
										    <button type="button" class="btn btn-primary btn-lg"
													onclick="javascript:updategiftexchangeNewpage('${sessionScope.user.user_guid}','${exchange.exchangeid}')">编辑</button>
		...
		<div class="pagination">
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/giftexchangemanage/giftexchangelist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
...
<div class="modal fade" id="uploadapprovepicModel" tabindex="-1" role="dialog"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group">
					    <input class="form-control" placeholder="userid" id="uploadapprovepic_userid"
							value="" style="display:none">
					</div>
					<div class="form-group">
					   <div>
							<input id="" type="button" value="设置认证照片" class="btn btn-info"
								data-toggle="modal" data-target="#titleImgModel" onclick="javascript:insertTitleImgHidden('15')">
						  </div>
						  <div>
							<input type="text" value="" id="approvepic"> <img
								id="approvepicshow" src=""
