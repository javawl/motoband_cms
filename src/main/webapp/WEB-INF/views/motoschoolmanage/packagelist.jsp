...
<body onload="">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">骑行学院系列列表&nbsp<button type="button" class="btn btn-success" data-toggle="modal"
		data-target="#addpackage"  onclick="javascript:addpackage()">添加系列</button></div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					...
					<div class="col-sm-3" style="padding-left: 0; margin-bottom: 15px">
                      <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1')">
								   ...
								   <th>orderindex<span>&nbsp</span>
									 <c:choose>
									  <c:when test="${order== '0' && orderConditions=='orderindex'}">
									     <a class="down" href="javascript:changeTriangle('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1','1','orderindex')" style="text-decoration:none;">▼</a>   
									  </c:when>
									  <c:when test="${order== '1'  && orderConditions=='orderindex'}">
									     <a class="up" href="javascript:changeTriangle('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1','0','orderindex')"   style="text-decoration:none;">▲</a>
									  </c:when>
									  <c:otherwise>
									     <a class="up_down" href="javascript:changeTriangle('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1','1','orderindex')"   style="text-decoration:none;">◊</a>									     
									  </c:otherwise>
									</c:choose> 
									</th>
									<th>pid<span>&nbsp</span>
									 <c:choose>
									  <c:when test="${order== '0' && orderConditions=='pid'}">
									     <a class="down" href="javascript:changeTriangle('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1','1','pid')" style="text-decoration:none;">▼</a>   
									  </c:when>
									  <c:when test="${order== '1'  && orderConditions=='pid'}">
									     <a class="up" href="javascript:changeTriangle('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1','0','pid')"   style="text-decoration:none;">▲</a>
									  </c:when>
									  <c:otherwise>
									     <a class="up_down" href="javascript:changeTriangle('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1','1','pid')"   style="text-decoration:none;">◊</a>									     
									  </c:otherwise>
									</c:choose> 
									</th>
									<th>title</th>
							...
							<tbody>
								<c:forEach items="${packagelist}" var="list"
									...
									<c:choose>
										...
										<td class="center">
											<button type="button" class="btn btn-success" data-toggle="modal" data-target="#editpackage" onclick="javascript:editpackage('${list.pid}')">编辑</button>
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5"/>&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/motoschoolmanage/packagelist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
	...
	<!-- Modal -->
	 <div class="modal fade" id="editpackage" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel">系列信息</h4>
				</div>
				<div class="modal-body" style="height:420px">
				  <div class="form-group">
					<div class="col-sm-6">
					    <label>pid</label>
						<input class="form-control" placeholder="" id="update_pid"
	...
	<script src="../js/motoschoolmanage/packagelist.js"></script> 
