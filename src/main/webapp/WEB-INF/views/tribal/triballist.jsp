...
function pageGoto(url, adminGuid,page){ 
	...
	$("#page-wrapper").load(url + "?userGuid=" + adminGuid+ "&page="+page+"&limit="+limit+"&order="+order+"&orderConditions="+orderConditions+"&tribaltype="+tribaltype);	
}
function inputPageGoto(url, adminGuid,totalPage){
	...
	if(reg.test(inputPage)==true && (parseInt(inputPage)<=parseInt(totalPage))){
		...
		var orderConditions= $("#triangleHiddenValue").val();
		var tribaltype= $("#tribaltypeSelect").val();
    	$("#page-wrapper").load(url + "?userGuid=" + adminGuid+ "&page="+inputPage+"&limit="+limit+"&order="+order+"&orderConditions="+orderConditions+"&tribaltype="+tribaltype);
...
<body >
   <div class="row">
	...
	<div class="row" >
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">热门部落列表 
				&nbsp&nbsp<button type="button" class="btn btn-info"   data-toggle="modal" data-target="#editTribalModel"  onclick="javascript:addTribal()">添加部落</button>
				&nbsp&nbsp<button type="button" class="btn btn-info" onclick="javascript:updateVersion('tribalversion')">更新部落版本</button>
				&nbsp<span style="color:red">关于热门部落的操作无需刷新版本号，操作完成立即生效！！！</span>
				</div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					<div class="dataTable_wrapper">
                          ...
                          <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','1')">
					...
					<div class="col-sm-3" style="padding-left: 0; margin-bottom: 15px">
                          <div>
							  ...
							  <select class="form-control" id="tribaltypeSelect"  onchange="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','1')">
									 <c:forEach items="${tribaltypelist}" var="tribaltypemodel" varStatus="ids">
										<c:choose>
											<c:when test="${tribaltypemodel.tribaltypeid==tribaltype }">
												<option value="${tribaltypemodel.tribaltypeid}" selected>${tribaltypemodel.name}</option>
											</c:when>
											<c:otherwise>
												<option value="${tribaltypemodel.tribaltypeid}">${tribaltypemodel.name}</option>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									<c:choose>
									  <c:when test="${tribaltype==100}">
                         ...
                         <div style="display:none;">
							 ...
							 <input id="tribaltypeHidden" type="hidden" value="${tribaltype }"/>
						...
						<table
							...
							<tbody>
								<c:forEach items="${triballists}" var="tribalmodel" varStatus="ids">
									<tr class="${tribalmodel.tribalid}">
									   <td>${tribalmodel.tribalid}</td>
									    <td>${tribalmodel.name}</td>
									    <td><img src='${tribalmodel.logopic}!thumb' width='100px' height='100px'></td>
									    <td><img src='${tribalmodel.bannerpic}!thumb' width='100px' height='100px'></td>
									    <td>${tribalmodel.des}</td>
									    <c:choose>
									       <c:when test="${tribalmodel.type=='0'}">
									          <td>地域</td>
									       </c:when>
									       <c:when test="${tribalmodel.type=='1'}">
									          <td>品牌</td>
									       </c:when>
									       <c:when test="${tribalmodel.type=='2'}">
									          <td>主题</td>
									       </c:when>
									       <c:otherwise>
									          <td>未知</td>
									    ...
									    <td>${tribalmodel.orderindex}</td>
										<td class="center">
										    
											<c:choose>
											  <c:when test="${tribaltype==100}">
											     <button type="button" class="btn btn-warning btn-lg"
										               data-toggle="modal" data-target="#changgeOrderindexModel"
														onclick="javascript:changgeOrderindex('${tribalmodel.tribalid}','${tribalmodel.orderindex}')">修改排序</button>
											     <button type="button" class="btn btn-warning btn-lg"
														onclick="javascript:deltribalfromhot('${tribalmodel.tribalid}')">从部落移除</button>
												 
											  </c:when>
											  <c:otherwise>
											     <button type="button" class="btn btn-primary btn-lg"
										               data-toggle="modal" data-target="#editTribalModel"
														onclick="javascript:editTribal('${tribalmodel.tribalid}')">编辑</button>
												 <button type="button" class="btn btn-primary btn-lg"
										               data-toggle="modal" data-target="#changgeOrderindexModel"
														onclick="javascript:changgeOrderindex('${tribalmodel.tribalid}','${tribalmodel.orderindex}')">修改排序</button>
											     <button type="button" class="btn btn-warning btn-lg"
										               data-toggle="modal" data-target="#addhottribalModel"
														onclick="javascript:addhottribal('${tribalmodel.tribalid}')">添加到热门</button>
											  </c:otherwise>
		...
		<div class="pagination">
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/tribal/triballist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/tribal/triballist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
	...
	<div class="modal fade" id="editTribalModel" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">部落详情</h4>
				</div>
				<div class="modal-body" style="height:700px">
				<div class="form-group">
				<input class="form-control" placeholder="" id="addtribalid"
							value="" type="hidden">
				...
				<input class="form-control" placeholder="" id="oldtype"
							value="" type="hidden">
				<label>名称</label>
				<input class="form-control" placeholder="请输入部落名称" id="addname"
							value="">	
				</div>
				<div class="form-group">
				<label>简介</label>
				<textarea rows="3" cols="" placeholder="请输入部落简介" id="adddes" style="width: 100%;"></textarea>
				</div>
				<div class="form-group">
				<label>主管理员</label>
				<textarea rows="3" cols="" placeholder="请输入主管理员userid，以英文逗号隔开" id="addmainadmins" style="width: 100%;"></textarea>
				</div>
				<div class="form-group">
				<label>副管理员</label>
				<textarea rows="3" cols="" placeholder="请输入副管理员userid，以英文逗号隔开" id="addviceadmins" style="width: 100%;"></textarea>
				</div>
				<div class="form-group">
					<div class="col-sm-6">
					    <label>logo图片</label>
				 ...
				 <div class="col-sm-12"  style="margin-top: 15px;">
                        <label>类型:</label>
                           <c:forEach items="${tribaltypelist}" var="tribaltypemodel" varStatus="ids">
                               <input type="radio" class="" placeholder="" name="addtype" value="${tribaltypemodel.tribaltypeid}" > ${tribaltypemodel.name}
                           </c:forEach>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" 
						onclick="javascript:editTribalConfirm()">确认</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<div class="modal fade" id="changgeOrderindexModel" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel">修改orderindex</h4>
				</div>
				<div class="modal-body" >
				<div class="form-group">
				<input class="form-control" placeholder="" id="addorderindex_tribalid"
							value="" type="hidden">
				<input class="form-control" placeholder="" id="addorderindex_type"
							value="" type="hidden">
				<label>orderindex(填数字，用于倒序排序，建议时间毫秒值)</label>
				<input class="form-control" placeholder="请输入orderindex" id="addorderindex"
							value="">	
				</div>
				</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal"
						id="closedelBoxModel">关闭</button>
					<button type="button" class="btn btn-primary" data-dismiss="modal"
						onclick="javascript:changgeOrderindexConfirm()">确认</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
    ...
    <script src="../js/consts.js"></script>
	<script src="../js/tribal/triballist.js"></script>
	...
	<script type="text/javascript">
            ...
            $('#editTribalModel').modal({backdrop: 'static', keyboard: false, show:false}); 
	</script> 
