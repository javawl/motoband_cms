...
function pageGoto(url, adminGuid,page){ 
	...
	var buserid= $("#buseridHidden").val();
	if(order==''||order == null){
		order=0;
	}	
	var orderConditions= $("#triangleHiddenValue").val();
  
	$("#commentlistdiv").load(url + "?userGuid=" + adminGuid+ "&page="+page+"&limit="+limit+"&order="+order+"&orderConditions="+orderConditions+"&buserid="+buserid);	
}
function inputPageGoto(url, adminGuid,totalPage){
	...
	if(reg.test(inputPage)==true && (parseInt(inputPage)<=parseInt(totalPage))){
		...
		var orderConditions= $("#triangleHiddenValue").val();
    	$("#commentlistdiv").load(url + "?userGuid=" + adminGuid+ "&page="+inputPage+"&limit="+limit+"&order="+order+"&orderConditions="+orderConditions+"&buserid="+buserid);
	...
	<div>
		  <div class="dataTable_wrapper">
                          ...
                          <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','1')">
                         ...
                         <div style="display:none;">
							 ...
							 <input id="buseridHidden" type="hidden" value="${buserid }"/>
						...
						<table
							...
							<tbody>
								 <c:forEach items="${commentlsit}" var="clist" varStatus="ids">
									<tr class="${clist.cid}">
									    <td>${clist.cid}</td>
										<td>${clist.userName}</td>
										<td>${clist.tuserName}</td>
										<td>${clist.content}</td>
										<td>${clist.score}</td>
										<td>${clist.modelName}</td>
										<td>${clist.cost}</td>
										<td class="center">
										    <button type="button" class="btn btn-danger btn-lg"
													onclick="javascript:deletbusercomment('${clist.cid}','${clist.tuserid }')">删除</button>
		...
		<div class="pagination">
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/businessmanage/businesscommentlistinfo','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
...
<div class="modal fade" id="changeorderindexModel" tabindex="-1" role="dialog"
    ...
    <script src="../js/consts.js"></script>
	<script src="../js/businessmanage/businesscommentlistinfo.js"></script>
