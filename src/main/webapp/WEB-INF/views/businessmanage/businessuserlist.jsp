...
<body >
   <div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">商品用户管理</h1>
		</div>
	</div>
	...
	<div class="row" >
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">商家用户列表 </div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					<div class="dataTable_wrapper">
                          ...
                          <div>
							  ...
							  <select class="form-control" id="pageSizeSelect"  onchange="javascript:pageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','1')">
						...
						<%-- <div class="col-sm-3" style="padding-left: 0; margin-bottom: 15px">
                          <div>
							  ...
							  <select class="form-control" id="stateSelect"  onchange="javascript:changestate('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','1',this)">
										<c:choose>
											<c:when test="${state==-1 }">
							...
							<tbody>
								<c:forEach items="${businessusers}" var="user" varStatus="ids">
									<tr class="${user.buid}">
										...
										<td>${user.staffcount}</td>
										<td>${user.age}</td>
										<td>${user.lbsid}</td>
										<c:choose>
										    <c:when test="${user.isapprove==0}"><td>否</td></c:when>
										    <c:when test="${user.isapprove==1}"><td>是</td></c:when>
										    <c:otherwise><td>未知</td></c:otherwise>
										</c:choose>
										<c:choose>
										    <c:when test="${user.state==0}"><td>否</td></c:when>
										    <c:when test="${user.state==1}"><td>是</td></c:when>
										    <c:otherwise><td>未知</td></c:otherwise>
										</c:choose>
										<td class="center">
										    <button type="button" class="btn btn-primary btn-lg"
													onclick="javascript:lookUserNewPage('${user.userid}')">查看用户信息</button>
										    <button type="button" class="btn btn-primary btn-lg"
										            data-toggle="modal" data-target="#resetpasswordModel"
													onclick="javascript:resetpassword('${user.userid}')">修改登录密码</button>
										    <c:choose>
										       <c:when test="${user.isapprove==0 }">
										          <button type="button" class="btn btn-info btn-lg"
													onclick="javascript:changetoapprove('${user.userid}')">设为认证商家</button>
										       </c:when>
										       <c:otherwise>
										          <button type="button" class="btn btn-info btn-lg"
													disabled="disabled">设为认证商家</button>
										       </c:otherwise>
										    </c:choose>
										     <button type="button" class="btn btn-primary btn-lg"
										            data-toggle="modal" data-target="#uploadapprovepicModel"
													onclick="javascript:uploadapprovepic('${user.userid}')">上传认证图片</button>
		...
		<div class="pagination">
			...
			<c:if test="${pageBean.page != 1 }">
				<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','1')">首页</a>
				<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','${pageBean.page-1}')">上一页</a>
			</c:if>
			   ...
			   <c:when test="${pageBean.totalPage >10}">
			       ...
			       <c:forEach var="i" begin="1" end="5">
				          ...
				          <c:when test="${pageBean.page != i }">
					          <a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','${i}')">${i}</a>
				          </c:when>
						  <c:otherwise>
			      ...
			      <c:if test="${pageBean.page  != pageBean.totalPage}">
						<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','${pageBean.page+1}')">下一页</a>
						<a class="btn btn-primary" href="javascript:pageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','${pageBean.totalPage}')">尾页</a>
			     </c:if>
			   </c:when>
			   <c:otherwise>
			...
			<span>&nbsp输入页数：</span>
			<span><input type="text" id="inputPage" size="5" />&nbsp<button type="button"  class="btn btn-success btn-lg" onclick="javascript:inputPageGoto('/businessmanage/businessuserlist','${sessionScope.user.user_guid}','${pageBean.totalPage}')" >跳转</button></span>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group">
					    <input class="form-control" placeholder="userid" id="resetpassword_userid"
							value="" style="display:none">
					</div>
					<div class="form-group">
						<input class="form-control" placeholder="password" id="resetpassword_password"
							value="" >
					</div>
