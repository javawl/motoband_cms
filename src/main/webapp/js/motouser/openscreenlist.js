...
function editopenscreen(osid){
	 var str="";
	  var datas =
		...
		{
		    type : "POST",
		    url : "../motouser/getOpenscreenById",
		    data : datas,
		    success : function (data)
		    {
			    if (data !=''&& data !=null)
			    {
			    	...
			    	$("#addosid").val(datajson.osid);
			    	$("#adduserid").val(datajson.userid);
			    	$("#addnickname").val(datajson.nickname);
                    $("#titleimg").val(datajson.pic);
			    	$("#titleimgshow").attr("src",datajson.pic)
			    }
...
function editopenscreenConfirm(){
  ...
  var datas =
	...
	{
	    type : "POST",
	    url : "../motouser/insertOrUpdateOpenscreen",
	    data : datas,
	    success : function (data)
	    {
		    if (data =='success')
		    {
		    	alert("修改成功");
		    	  ...
		    	  $("#page-wrapper").load ("/motouser/openscreenlist?page=1&limit=20&order=0");
		    }else{
		    	alert("修改失败");
		    }
