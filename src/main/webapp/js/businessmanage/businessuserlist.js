...
function XX_selectImg (XX_img_guid)
{
	...
	else
	{
		...
		if(flag=='1'){
		   ...
		   $ ("#activitypics11").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='112'){
			$ ("#activitypics12").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='113'){
			$ ("#activitypics13").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='114'){
			$ ("#activitypics14").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='121'){
		   $ ("#activitypics21").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='122'){
			$ ("#activitypics22").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='123'){
			$ ("#activitypics23").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='124'){
			$ ("#activitypics24").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='131'){
		   $ ("#activitypics31").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='132'){
			$ ("#activitypics32").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='133'){
			$ ("#activitypics33").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='134'){
			$ ("#activitypics34").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='141'){
		   $ ("#activitypics41").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='142'){
			$ ("#activitypics42").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='143'){
			$ ("#activitypics43").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='144'){
			$ ("#activitypics44").val ($ ('#XX_' + img_guid).find ("img").attr ("src"));
		}else if(flag=='15'){
...
function lookUserNewPage(userid){
	window.open("../businessmanage/lookUserNewPage?userid="+userid);
}
	...
	$.ajax (
	{
	    ...
	    success : function (data)
	    {
		    if (data!=null && data!='')
		    {
			    ...
			    $("#look_userid").val(jsondata.userid);
			  ...
			  //  $("#look_type").val(jsondata.type);
			    ...
			    $("input[name='look_state'][value='"+jsondata.state+"']").prop("checked",true);
			  ...
			  //  $("#look_businessserviceliststr").val(jsondata.businessserviceliststr);
		    	...
		    	if(serviceids_arr.length>0){
		    		for(var i=0;i<serviceids_arr.length;i++){
		    			...
		    			var servicelist=jsondata.businessservicelist;
		    			for(var j=0;j<servicelist.length;j++){
		    				if(servicelist[j].bsid==serviceids_arr[i]){
		    					$("#look_servicecontent"+serviceids_arr[i]).val(servicelist[j].content);
		    				}
		    			}
		    			
		    			$("#servicecontentDiv"+serviceids_arr[i]).show();
		    		}
			    ...
			    $("#look_lbsid").val(jsondata.lbsid);
			    $("#look_officestarttime").val(jsondata.officestarttime);
			    $("#look_officeendtime").val(jsondata.officeendtime);
			    $("#look_busikeyword").val(jsondata.busikeyword);
			    $("#look_kfuseridlist").val(jsondata.kfuseridlist);
			    $("#look_des").val(jsondata.des);
			    $("#look_desdetail").val(jsondata.desdetail);
			    $("input[name='look_isapprove'][value='"+jsondata.isapprove+"']").prop("checked",true);
			    $("#look_businesscode").val(jsondata.businesscode);
			    $("#headurl").val(jsondata.headurl);
			    $("#headurlshow").attr("src",jsondata.headurl);
			    if(jsondata.doorpics!=null && jsondata.doorpics !=''){
			    	var doorpicarr=jsondata.doorpics.split(",");
			    	for(var i=0;i<doorpicarr.length;i++){
			    		 $("#doorpics"+(i+1)).val(doorpicarr[i]);
						 $("#doorpicsshow"+(i+1)).attr("src",doorpicarr[i]);
			    	}
			    }
			    ...
			    if(jsondata.businessactivitylist!=null &&jsondata.businessactivitylist!='' ){
			    	var activitylist=jsondata.businessactivitylist;
			    	for(var i=0; i<activitylist.length;i++){
			    		$("#look_activitybaid"+(i+1)).val(activitylist[i].baid);
			    		$("#look_activitytitle"+(i+1)).val(activitylist[i].title);
			    		$("#look_activitycontent"+(i+1)).val(activitylist[i].content);
			    		//$("#activitypics"+(i+1)).val(activitylist[i].pics);
						//$("#activitypicsshow"+(i+1)).attr("src",activitylist[i].pics);
			    		var picsarr=activitylist[i].pics.split(",");
				    	for(var j=0;j<picsarr.length;j++){
				    		 $("#activitypics"+(i+1)+""+(j+1)).val(picsarr[j]);
							 $("#activitypicsshow"+(i+1)+""+(j+1)).attr("src",picsarr[j]);
				    	}
			    	}
			    }
	         ...
	         for(var i=0;i<jsondata.usecarmaingroupModels.length;i++){
	        	 str+='<option value="'+jsondata.usecarmaingroupModels[i].groupid+'">'+jsondata.usecarmaingroupModels[i].name+'</option>';
	         }
...
//		}
	...
	function editUserConfirm(){
		...
		/* var state= $("#look_state").val();
		 var reason= $("#look_reason").val();
		 var addtime= $("#look_addtime").val();
		 var businesscode= $("#look_businesscode").val();*/
		 
		 var businessservice_arr=[];
		 var businessservice_descarr=[];
		  $("input[name='checkbox_businessservice']:checked").each(function() {
			  businessservice_arr.push($(this).val());
			  var temp=$("#look_servicecontent"+$(this).val()).val();
//			  console.log(temp);
...
//			  console.log(typeof(temp));
			  ...
			  businessservice_descarr.push(temp);
		    });
	   
	    
	    var officestarttime= $("#look_officestarttime").val();
	    var officeendtime= $("#look_officeendtime").val();
	    var busikeyword= $("#look_busikeyword").val();
	    var kfuseridlist= $("#look_kfuseridlist").val();
	    var des= $("#look_des").val();
	    var desdetail= $("#look_desdetail").val();
	    var isapprove= $("input[name='look_isapprove']:checked").val();
	    var state= $("input[name='look_state']:checked").val();
	    var headurl= $("#headurlshow").attr("src");
	    
	    var doorpics1= $("#doorpics1").val();
	    var doorpics2= $("#doorpics2").val();
	    var doorpics3= $("#doorpics3").val();
	    var doorpics4= $("#doorpics4").val();
	    
	    var shoppics1= $("#shoppics1").val();
	    var shoppics2= $("#shoppics2").val();
	    var shoppics3= $("#shoppics3").val();
	    var shoppics4= $("#shoppics4").val();
	    
	    var activitybaid1= $("#look_activitybaid1").val();
	    var activitytitle1= $("#look_activitytitle1").val();
	    var activitycontent1= $("#look_activitycontent1").val();
	    var activitypics11= $("#activitypics11").val();
	    var activitypics12= $("#activitypics12").val();
	    var activitypics13= $("#activitypics13").val();
	    var activitypics14= $("#activitypics14").val();
	    
	    var activitybaid2= $("#look_activitybaid2").val();
	    var activitytitle2= $("#look_activitytitle2").val();
	    var activitycontent2= $("#look_activitycontent2").val();
	    var activitypics21= $("#activitypics21").val();
	    var activitypics22= $("#activitypics22").val();
	    var activitypics23= $("#activitypics23").val();
	    var activitypics24= $("#activitypics24").val();
	    
	    var activitybaid3= $("#look_activitybaid3").val();
	    var activitytitle3= $("#look_activitytitle3").val();
	    var activitycontent3= $("#look_activitycontent3").val();
	    var activitypics31= $("#activitypics31").val();
	    var activitypics32= $("#activitypics32").val();
	    var activitypics33= $("#activitypics33").val();
	    var activitypics34= $("#activitypics34").val();
	    
	    var activitybaid4= $("#look_activitybaid4").val();
	    var activitytitle4= $("#look_activitytitle4").val();
	    var activitycontent4= $("#look_activitycontent4").val();
	    var activitypics41= $("#activitypics41").val();
	    var activitypics42= $("#activitypics42").val();
	    var activitypics43= $("#activitypics43").val();
	    var activitypics44= $("#activitypics44").val();
	    ...
	    if(activitybaid1!=null && activitybaid1!='' ){
	    	 if(activitytitle1==null || activitytitle1==''||activitycontent1==null ||activitycontent1=='' ){
			    	alert("商家活动1中有数据为空");
			    	 $("#back").css("display","none");
			    	return ;
			    }
	    }
	    if(activitybaid2!=null && activitybaid2!='' ){
	    	if(activitytitle2==null || activitytitle2==''||activitycontent2==null ||activitycontent2=='' ){
		    	alert("商家活动2中有数据为空");
		    	 $("#back").css("display","none");
		    	return ;
		    }
	    }
	    if(activitybaid3!=null && activitybaid3!='' ){
	    	 if(activitytitle3==null || activitytitle3==''||activitycontent3==null ||activitycontent3==''){
			    	alert("商家活动3中有数据为空");
			    	 $("#back").css("display","none");
			    	return ;
			    }
	    }
	    if(activitybaid4!=null && activitybaid4!='' ){
	        if(activitytitle4==null || activitytitle4==''||activitycontent4==null ||activitycontent4=='' ){
		    	alert("商家活动4中有数据为空");
		    	$("#back").css("display","none");
		    	return ;
		    }
	    ...
	    var datas =
	    	{
	    	    ...
	    	    "businessservice_arr":businessservice_arr,
	    	    "businessservice_descarr":businessservice_descarr,
	    	    ...
	    	    "shoppics3":shoppics3,
	    	    "shoppics4":shoppics4,
	    	    "activitybaid1":activitybaid1,
	    	    "activitytitle1":activitytitle1,
	    	    "activitycontent1":activitycontent1,
	    	    "activitypics11":activitypics11,
	    	    "activitypics12":activitypics12,
	    	    "activitypics13":activitypics13,
	    	    "activitypics14":activitypics14,
	    	    "activitybaid2":activitybaid2,
	    	    "activitytitle2":activitytitle2,
	    	    "activitycontent2":activitycontent2,
	    	    "activitypics21":activitypics21,
	    	    "activitypics22":activitypics22,
	    	    "activitypics23":activitypics23,
	    	    "activitypics24":activitypics24,
	    	    "activitybaid3":activitybaid3,
	    	    "activitytitle3":activitytitle3,
	    	    "activitycontent3":activitycontent3,
	    	    "activitypics31":activitypics31,
	    	    "activitypics32":activitypics32,
	    	    "activitypics33":activitypics33,
	    	    "activitypics34":activitypics34,
	    	    "activitybaid4":activitybaid4,
	    	    "activitytitle4":activitytitle4,
	    	    "activitycontent4":activitycontent4,
	    	    "activitypics41":activitypics41,
	    	    "activitypics42":activitypics42,
	    	    "activitypics43":activitypics43,
	    	    "activitypics44":activitypics44
	    	   
	    	}
	    	$.ajax (
	    	{
	    	    type : "POST",
	    	    url : "../businessmanage/updateBUser",
	    	    data : datas,
	    	    success : function (data)
	    	    {
	    		    if (data=='success')
