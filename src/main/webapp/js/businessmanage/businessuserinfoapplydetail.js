...
function divshow(flag){
	if(flag==1){
		$("#baseinfoDiv").show();
		$("#environmentalDiv").hide();
		$("#businessservicelistDiv").hide();
		$("#businessactivitylistDiv").hide();
	}else if(flag==2){
		$("#baseinfoDiv").hide();
		$("#environmentalDiv").show();
		$("#businessservicelistDiv").hide();
		$("#businessactivitylistDiv").hide();
	}else if(flag==3){
		$("#baseinfoDiv").hide();
		$("#environmentalDiv").hide();
		$("#businessservicelistDiv").show();
		$("#businessactivitylistDiv").hide();
	}else if(flag==4){
		$("#baseinfoDiv").hide();
		$("#environmentalDiv").hide();
		$("#businessservicelistDiv").hide();
		$("#businessactivitylistDiv").show();
	}
}
function changeuserinfoapplystate(type,state){
    var buserid=$("#hiddenValue").val();
    var reason='';
    if(type==1){
    	reason=$("#baseinfo_reason").val();
    }else if(type==2){
    	reason=$("#environmental_reason").val();
    }else if(type==3){
    	reason=$("#servicelist_reason").val();
    }else if(type==4){
    	reason=$("#activitylist_reason").val();
    }
    if(state==2){
    	if(reason==null || reason==''){
    		alert("拒绝的原因不能为空，请输入拒绝原因");
    		return false;
    	}
    }
   
    if (confirm ("确认要修改该商家的信息审核状态么，此操作不可逆,谨慎操作？")){
	var datas =
	{
	    "reason" : reason,
	    "buserid" : buserid,
	...
	{
	    type : "POST",
	    url : "../businessmanage/changeUserinfoApplyState",
	    data : datas,
	    success : function (data)
	    {
		    if (data=='success')
